...
model_GRU = Sequential(name = 'GRU_Model', layers = [
        Input(shape=(n_timesteps, len(X_train_scaled.keys()) )),
...
early_stopping = EarlyStopping(monitor='val_loss', patience=15, restore_best_weights=True)
history = model_GRU.fit(X_np, y_np, epochs=50, validation_data = (X_np_test,y_np_test), callbacks=[early_stopping])
...
MyFunction.AffichageResLSTM(model_GRU, X_np, X_np_test, y_np, y_np_test, scaler_y)
